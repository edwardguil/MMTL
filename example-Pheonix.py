...
from MMTL import VisualModule, LanguageModule, EndToEndModule, load_weights
...
def main(weights):
    ...
    dataset = PheonixDataset(root, split="train", mean=mean, std=std)
    dataloader = DataLoader(dataset, batch_size=5, collate_fn=partial(collate_fn, device=device), shuffle=True)
    datasetEval = PheonixDataset(root, split="val", mean=mean, std=std)
